...
def target_encoding(train_data, test_data, categorical_cols, target_col='price', min_samples=10):
    """目标编码"""
    print("=== 目标编码开始 ===")
...
def detect_and_handle_outliers(data, target_col='price'):
    """检测和处理异常值"""
    print("=== 异常值处理开始 ===")
    ...
    data_clipped[target_col] = data_clipped[target_col].clip(lower_bound, upper_bound)
    
    # 方法2：移除异常值
    data_cleaned = data[(data[target_col] >= lower_bound) & (data[target_col] <= upper_bound)]
    
    print(f"裁剪后数据形状: {data_clipped.shape}")
    print(f"清理后数据形状: {data_cleaned.shape}")
    
    # 返回清理后的数据（移除异常值）
    return data_cleaned

def advanced_feature_engineering(all_data, train_data):
    """高级特征工程"""
    print("=== 高级特征工程开始 ===")
    
    # 1. 多项式特征
    if 'power' in all_data.columns and 'kilometer' in all_data.columns:
        all_data['power_km_poly2'] = all_data['power'] ** 2 + all_data['kilometer'] ** 2
        all_data['power_km_poly3'] = all_data['power'] ** 3 + all_data['kilometer'] ** 3
    ...
    if v_cols:
        # 手动计算平均绝对偏差
        all_data['v_mad'] = all_data[v_cols].sub(all_data[v_cols].mean(axis=1), axis=0).abs().mean(axis=1)
        all_data['v_cv'] = all_data[v_cols].std(axis=1) / (all_data[v_cols].mean(axis=1) + 1e-8)  # 变异系数
    
    # 4. 交互特征增强
    if 'power' in all_data.columns and 'car_age' in all_data.columns and 'kilometer' in all_data.columns:
        all_data['power_age_km_ratio'] = all_data['power'] / (all_data['car_age'] * all_data['kilometer'] + 1)
        all_data['power_age_km_sum'] = all_data['power'] + all_data['car_age'] + all_data['kilometer']
        all_data['power_age_km_product'] = all_data['power'] * all_data['car_age'] * all_data['kilometer']
    
    # 5. 品牌-车型-地区组合特征
    if all([col in all_data.columns for col in ['brand', 'model', 'regionCode']]):
        all_data['brand_model_region'] = all_data['brand'] * 10000 + all_data['model'] * 100 + all_data['regionCode']
    
    # 6. 时间特征增强
    if 'regYear' in all_data.columns:
        all_data['regYear_squared'] = all_data['regYear'] ** 2
        all_data['regYear_cubed'] = all_data['regYear'] ** 3
    
    if 'creatYear' in all_data.columns:
        all_data['creatYear_squared'] = all_data['creatYear'] ** 2
        all_data['creatYear_cubed'] = all_data['creatYear'] ** 3
    
    # 7. 价格相关特征（基于训练集统计）
    if 'brand' in all_data.columns:
        brand_price_stats = train_data.groupby('brand')['price'].agg([
            'mean', 'std', 'median', 'min', 'max', 'count'
        ]).reset_index()
        brand_price_stats.columns = ['brand', 'brand_price_mean', 'brand_price_std', 
                                   'brand_price_median', 'brand_price_min', 'brand_price_max', 'brand_count']
        
        all_data = all_data.merge(brand_price_stats, on='brand', how='left')
        
        # 填充缺失值
        for col in brand_price_stats.columns[1:]:
            if col in all_data.columns:
                all_data[col].fillna(all_data[col].median(), inplace=True)
    
    if 'model' in all_data.columns:
        model_price_stats = train_data.groupby('model')['price'].agg([
            'mean', 'std', 'median', 'min', 'max', 'count'
        ]).reset_index()
        model_price_stats.columns = ['model', 'model_price_mean', 'model_price_std', 
                                   'model_price_median', 'model_price_min', 'model_price_max', 'model_count']
        
        all_data = all_data.merge(model_price_stats, on='model', how='left')
        
        # 填充缺失值
        for col in model_price_stats.columns[1:]:
            if col in all_data.columns:
                all_data[col].fillna(all_data[col].median(), inplace=True)
...
def load_and_preprocess_data():
    ...
    train_data, test_data = target_encoding(train_data, test_data, categorical_cols)
    
    # 异常值处理
    train_data = detect_and_handle_outliers(train_data)
    ...
    # 高级特征工程
    all_data = advanced_feature_engineering(all_data, train_data)
    ...
    if 'power' in all_data.columns:
        ...
        all_data['power_bin_small'] = pd.cut(all_data['power'], bins=5, labels=False).fillna(0)
    ...
    if 'kilometer' in all_data.columns:
        ...
        all_data['kilometer_bin_small'] = pd.cut(all_data['kilometer'], bins=5, labels=False).fillna(0)
...
def train_and_predict(X_train, y_train, X_test, test_saleids, transform_type):
    ...
    params = {
        ...
        'n_estimators': 4000,  # 增加树的数量
        'subsample': 0.85,  # 调整采样
        'colsample_bytree': 0.85,  # 调整特征采样
        'min_child_weight': 2,  # 降低最小子节点权重
        'reg_alpha': 0.05,  # 降低L1正则化
        'reg_lambda': 0.8,  # 降低L2正则化
        'random_state': 42,
        'verbosity': 0
    }
    
    print("训练 XGBoost (激进参数)...")
    model = xgb.XGBRegressor(**params)
    try:
        model.fit(X_tr, y_tr, eval_set=[(X_val, y_val)], early_stopping_rounds=150, verbose=100)
    except TypeError:
    ...
    submission.to_csv('aggressive_optimization_submission.csv', index=False)
    
    print(f"\n=== 最终结果 ===")
    print(f"XGBoost 验证集MAE: {val_mae:.2f}")
    print("预测结果已保存到: aggressive_optimization_submission.csv")
    
    return val_mae, submission

if __name__ == "__main__":
    print("=== 激进优化XGBoost - 冲击400分 ===")
