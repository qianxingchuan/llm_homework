...
def load_and_preprocess_data():
    """加载和预处理数据 - 基于GPU训练脚本的完整特征工程 + 对数变换"""
    print("=== 最终优化特征工程开始 ===")
    ...
    y_train = transformed_target  # 使用对数变换后的目标变量
...
def train_and_predict(X_train, y_train, X_test, test_saleids, transform_type):
    ...
    submission.to_csv('final_optimized_xgb_submission.csv', index=False)
    
    print(f"\n=== 最终结果 ===")
    print(f"XGBoost 验证集MAE: {val_mae:.2f}")
    print("预测结果已保存到: final_optimized_xgb_submission.csv")
    
    return val_mae, submission

if __name__ == "__main__":
    print("=== 最终优化XGBoost - 冲击400分 ===")
