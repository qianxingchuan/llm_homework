...
def load_and_preprocess_data():
    ...
    return X_train, y_train, X_test, original_test_saleids

def train_and_predict(X_train, y_train, X_test, test_saleids):
    """训练XGBoost和LightGBM，集成预测"""
    ...
    print(f"XGBoost 验证集MAE: {xgb_mae:.2f}")
    ...
    print(f"LightGBM 验证集MAE: {lgb_mae:.2f}")
    ...
    submission.to_csv('optimized_gpu_submission.csv', index=False)
    ...
    print(f"\n预估集成后MAE: {ensemble_mae:.2f}")
    print("预测结果已保存到: optimized_gpu_submission.csv")
    
    return val_scores, submission

if __name__ == "__main__":
    print("=== 优化GPU训练 - 冲击400分 ===")
    X_train, y_train, X_test, test_saleids = load_and_preprocess_data()
    val_scores, submission = train_and_predict(X_train, y_train, X_test, test_saleids)
    print(f"最终验证集MAE: {sum(val_scores.values())/len(val_scores):.2f}") 