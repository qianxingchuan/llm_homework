...
def train_and_predict(X_train, y_train, X_test, test_saleids, transform_type):
    ...
    params = {
        ...
        'n_estimators': 1500,  # 适中的树数量
        'subsample': 0.8,
        'colsample_bytree': 0.8,
        'min_child_weight': 5,  # 更高的最小子节点权重
        'reg_alpha': 0.2,  # 更高的正则化
        'reg_lambda': 1.5,
        'random_state': 42,
        'verbosity': 0
    }
    
    print("训练 XGBoost (最保守参数)...")
    ...
    submission.to_csv('conservative_baseline_submission.csv', index=False)
    
    print(f"\n=== 最终结果 ===")
    print(f"XGBoost 验证集MAE: {val_mae:.2f}")
    print("预测结果已保存到: conservative_baseline_submission.csv")
